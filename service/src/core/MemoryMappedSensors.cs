...
#pragma warning restore CS8618  // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring
                                // as nullable.
        {
            try {
                ...
                if (Config.FeatureEnableMemoryMapAllHardwareData) {
                    mmfAllHardware = MemoryMappedFileFactory.CreateNew(
                        Constants.FILENAME_ALLHARDWARE, MMAP_SIZE * 1024, MemoryMappedFileAccess.ReadWrite,
                        MemoryMappedFileOptions.None, security, System.IO.HandleInheritability.Inheritable);
        ...
        public void writeSensors(byte[] index, byte[] data, Metadata metadata) {
            try {
                ...
                // a buggy client left a mutex and didn't release, we should ignore the exception (only this service writes to the
                // memory map file)
            }
            try {
                ...
                int indexOffset =
                    metadataBlockSize + reservedBytes.Length + (headerFieldsOffsets[headerFieldsOffsets.Length - 1] * 4);
            ...
            try {
                ...
                // if we catch the exception in the WaitOne when a client don't release the mutex, we can get a another here, so
                // ignore it too
            }
        }
        ...
        public void writeHardware(byte[] data, Metadata metadata) {
            if (Config.FeatureEnableMemoryMapAllHardwareData) {
                try {
                    ...
                    // a buggy client left a mutex and didn't release, we should ignore the exception (only this service writes to
                    // the memory map file)
                }
